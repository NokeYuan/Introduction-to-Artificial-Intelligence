...
class ExactInference(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        "Begin with a uniform distribution over ghost positions.python busters.py"
        ...
        * When a ghost is captured by Pacman, all beliefs should be
        ...
        if noisyDistance == None:
            ...
            for ghostposition in self.legalPositions:
                dist = util.manhattanDistance(ghostposition, pacmanPosition)
                allPossible[ghostposition] = emissionModel[dist] * self.beliefs[ghostposition]
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            particles = [ghostposition]*self.numParticles
            self.particles = particles

        if noisyDistance != None:
            for i in self.particles:
                dist = util.manhattanDistance(i,pacmanPosition)
                allPossible[i] = allPossible[i] + emissionModel[dist]
            ...
            if allPossible.totalCount() > 0:
                particles = []
                particalnum = self.numParticles
                for i in range(particalnum):
    ...
    def elapseTime(self, gameState):
        ...
        while i < len(self.particles):
            newPosDis = self.getPositionDistribution(self.setGhostPosition(gameState, self.particles[i]))
            pos = util.sample(newPosDis)
            newPar.append(pos)
            i += 1
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        "*** YOUR CODE HERE ***"
        self.particles = []
        tuple_list = list(itertools.product(self.legalPositions,repeat=self.numGhosts))
    ...
    def observeState(self, gameState):
        ...
        for p in self.particles:
            ...
            for i in range(self.numGhosts):
                ...
                if noisyDistances[i] != None:
                    prob = prob * emissionModels[i][util.manhattanDistance(p[i],pacmanPosition)]
