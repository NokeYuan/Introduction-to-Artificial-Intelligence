...
class QueensTableConstraint(TableConstraint):
    ...
    def __init__(self, name, qi, qj, i, j):
        ...
        cor_assignment = []
        for each_i in qi.domain():
            for each_j in qj.domain():
                if each_i != each_j:
                    if abs(each_i - each_j) != abs(i - j):
                        cor_assignment.append([each_i,each_j])
        TableConstraint.__init__(self,name,[qi,qj],cor_assignment)
...
class NValuesConstraint(Constraint):
    ...
    def hasSupport(self, var, val):
        ...
        def full_check(l):
            required_values = self._required
            occurrence = 0
            vals = [val for (var, val) in l]
            for val in vals:
                if val in required_values:
                    occurrence += 1
            if occurrence >= self._lb:
                if occurrence <= self._ub:
                    return True
            return False

        varsToAssign = self.scope()
        varsToAssign.remove(var)
        x = findvals(varsToAssign, [(var, val)], full_check)
        return x
